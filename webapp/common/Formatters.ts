...
export function formatErrorDetails(status: string, errorMessage: string): string {
    switch (status as SystemStatus) {
        case SystemStatus.FAILED:
            ...
            return getText('errorSystemFailed');
...
export function isAutogeneratedGroup(name: string): boolean {
    if (!name) return false;
    return name.includes('TenantAdministrator') || name.includes('TenantAuditor');
}

export function isGroupDeleteEnabled(name: string): boolean {
    return !isAutogeneratedGroup(name);
}

export function isGroupEditEnabled(name: string): boolean {
    return !isAutogeneratedGroup(name);
}

export function autogeneratedGroupTooltip(name: string): string {
    if (isAutogeneratedGroup(name)) {
        return getText('autogeneratedGroupActionDisabledTooltip');
    }
    return '';
}
