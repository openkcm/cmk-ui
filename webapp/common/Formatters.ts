...
import { TaskStates, SystemStatus, SystemType, GroupRoles, KeyStates, StateColors, KeyCreationTypes, WorkflowStatus, ActionTypes, ArtifactTypes, ProtectedGroups } from './Enums';
import { getText } from 'kms/common/Helpers';
import { AWSAccessDetails } from './Types';

const PROTECTED_GROUPS: string[] = Object.values(ProtectedGroups);
...
export function isGroupDeletable(name: string): boolean {
    return !PROTECTED_GROUPS.includes(name);
}

export function getGroupDeleteTooltip(name: string, deleteText: string, autoGeneratedText: string): string {
    return PROTECTED_GROUPS.includes(name) ? autoGeneratedText : deleteText;
}