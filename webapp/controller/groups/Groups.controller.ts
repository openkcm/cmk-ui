...
import { isAutogeneratedGroup } from 'kms/common/Formatters';
...
export default class Group extends BaseController {
    ...
    public onKeyTableDeletePress(event: ListItemBase$PressEvent): void {
        ...
        const selectedGroup = this.oneWayModel.getProperty(path) as { id: string, name: string };

        if (isAutogeneratedGroup(selectedGroup.name)) {
            return;
        }
